...
describe('Proper typedef types generation', () => {
  it('Interface type Validation', async () => {
    CheckTypeFields(
      ...
      (Type: string) => DocumentApi.getInterfaceType(`User${Type}Interface`)
    );
  });
  ...
  it('Object type validation', async () => {
    ['Mixed', 'Others', 'Self'].forEach((auth) => {
      CheckTypeFields(
        ...
        (Type: string) => DocumentApi.getObjectType(`${auth}${Type}Object`)
      );
    });
  });
  ...
  it('Input type validation', async () => {
    CheckTypeFields(
      ...
      (Type: string) => DocumentApi.getInputType(`Create${Type}Input`)
    );
    CheckTypeFields(
      [
      ...
      (Type: string) => DocumentApi.getInputType(`Update${Type}Input`)
    );
  });
  ...
  it('Union type validation', async () => {
    ['User', 'UserField3', 'UserField3Field32'].forEach((UnionStr) => {
      ...
      const UnionValue = ['Mixed', 'Others', 'Self']
        .reduce((prev, next) => prev.concat(`${next}${UnionStr}Object`), [] as any[])
        .join('|');
